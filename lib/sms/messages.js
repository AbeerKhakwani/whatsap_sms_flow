...
Voice note, text, or photos — whatever's easiest!`,
    ...
    SELL_EXTRACTED: (listing, payout, missing) => {
        ...
        if (listing.details) response += `• ${listing.details}\n`;
    ...
    SELL_ASK_DETAILS: `Any flaws to disclose? 📝

Missing buttons, stains, alterations, wear?

Reply SKIP if none!`,

    SELL_ASK_DESCRIPTION: (listing) => `Photos received! 📸

Now tell me more about this ${listing.item_type || 'item'}:
• Color & fabric?
• Any flaws or wear?
• Special details (embroidery, stonework)?

This helps it sell faster! Or reply SKIP.`,

    SELL_ASK_LINK: `Almost done! 🎉
...
Now send me 3+ photos — one at a time please! 📸
(Include the brand tag if you have it)`,
    ...
    SELL_PHOTO_RECEIVED: (count, feedback) => {
        ...
        if (count >= 3) {
            ...
            response += `Photo ${count} received! ✓\n\nSend ${needed} more (one at a time) 📸`;
        }
        return response;
    },
...
Size ${listing.size || ''} • ${listing.condition || ''}
Asking: $${listing.asking_price_usd || ''}
Photos: ${(listing.photo_urls?.length || 0) + (listing.photo_tag_url ? 1 : 0)} ✓${listing.original_link ? `\nLink: ✓` : ''}
━━━━━━━━━━━━━━━━
...
Example: "Sana Safinaz 3-piece, medium, like new, $85"`,
    ...
    SELL_NO_CUSTOM: `We only sell Pakistani designer brands — not custom/handmade items. 💛

Which Pakistani designer made this? (e.g., Sana Safinaz, Elan, Maria B, Khaadi, etc.)`,

    SELL_NO_INDIAN: `We specialize in Pakistani designers only! 🇵🇰

Which Pakistani brand is this? (e.g., Sana Safinaz, Elan, Maria B, Khaadi, Sapphire, etc.)`,

    // ============ OFFER FLOW ============
    OFFER_START: `Making offers is coming soon! 🚀

For now, browse our collection at thephirstory.com

Want to be notified when this feature launches? Just reply YES!`,
...
Reply SELL to get started 💛`;
        ...
        let response = `Your listings (${listings.length}): 📦\n\n`;

        listings.forEach((item, i) => {
            const status = item.isSold ? '✅ SOLD' : item.status === 'active' ? '🟢 Live' : '🟡 Pending';
            response += `${i + 1}. ${item.title}\n`;
            response += `   $${item.price} • ${status}\n`;
            if (item.isSold) {
                response += `   You earned: $${item.sellerPayout?.toFixed(0) || '0'}\n`;
            } else {
                response += `   You'll get: $${item.sellerPayout?.toFixed(0) || '0'}\n`;
            }
            response += `\n`;
        });

        response += `View all: sell.thephirstory.com\n\nReply SELL to list more!`;
        return response;
    },

    // ============ SALES FLOW ============
    SALES_START: `Pulling up your sales... ⏳`,

    SALES_EMPTY: `No sales yet!

Keep promoting your listings — they'll sell! 💪

Reply LISTINGS to see your active items.`,

    SALES_SHOW: (sales, totalEarnings, pendingPayout) => {
...
Reply LISTINGS to see your active items.`;
        ...
        let response = `Your sales (${sales.length}): 🎉\n\n`;

        sales.forEach((item, i) => {
            const status = item.status === 'SOLD_WITH_PAYOUT' ? '✅ Paid' : '⏳ Pending';
            response += `${i + 1}. ${item.title}\n`;
            response += `   Sold: $${item.retailPrice} • ${status}\n`;
            response += `   You earned: $${item.earnings?.toFixed(0) || '0'}\n\n`;
        });

        response += `━━━━━━━━━━━━━━━━\n`;
        response += `💰 Total Earned: $${totalEarnings?.toFixed(0) || '0'}\n`;
        if (pendingPayout > 0) {
            response += `⏳ Pending Payout: $${pendingPayout?.toFixed(0)}\n`;
        }
        response += `━━━━━━━━━━━━━━━━\n\n`;

        response += `Reply SELL to list more items!`;
        return response;
    },
};
