 ...
 * @param {Array<{type: string, id: string, mime: string}>} mediaItems - Array of media objects with IDs
 * @param {string} sellerId - Seller ID for organizing files
 * @param {string} messageId - WhatsApp message ID for unique filenames
 * @returns {Promise<string[]>} - Array of Supabase public URLs
 */
export async function processWhatsAppMedia(mediaItems, sellerId, messageId) {
  ...
  for (let i = 0; i < mediaItems.length; i++) {
    try {
      ...
      console.log(`📥 Getting WhatsApp media URL for ${media.id}...`);
      ...
      console.log(`📥 Downloading media ${i + 1}/${mediaItems.length}...`);
