    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a classifier for a clothing resale SMS service. Classify the user's intent into exactly one of: sell, offer, listings, unknown.
...
Respond with ONLY the intent word, nothing else.`
    ...
    if (['sell', 'offer', 'listings'].includes(intent)) {
      return intent;
    }
    return 'unknown';
  } catch (error) {
