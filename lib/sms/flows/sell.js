...
export async function handleSellFlow(message, conversation, seller, mediaUrls = []) {
  ...
  if (state === 'sell_form_price') {
    ...
    await setState(conversation.id, 'sell_form_details', { listing_id: listingId });
    return msg('SELL_GOT_IT') + '\n\n' + msg('SELL_ASK_DETAILS');
  }

  if (state === 'sell_form_details') {
    const skipPhrases = ['skip', 'none', 'no', 'n/a', 'na', 'nothing', 'nope'];
    if (!skipPhrases.includes(lower)) {
  ...
  if (state === 'sell_awaiting_item_photos') {
    ...
    if (mediaUrls.length === 0) {
      ...
      if (currentCount >= 3) {
        ...
        const lastSummary = context.summary_sent_at;
        if (lastSummary && (Date.now() - lastSummary) < 10000) {
          return null; // Skip, already sent summary
        }
        await setState(conversation.id, 'sell_confirming', { listing_id: listingId, summary_sent_at: Date.now() });
        return msg('SELL_SUMMARY', listing) + '\n\n' + msg('SELL_CONFIRM').text;
      }
    ...
    // Check if summary was already sent recently (prevent duplicates from rapid photo batches)
    const lastSummary = context.summary_sent_at;
    if (lastSummary && (Date.now() - lastSummary) < 10000) {
      return null; // Skip, already sent summary
    }
...
async function submitListing(listing, conversation, seller) {
  try {
    ...
    const shopifyProduct = await createDraft({
      ...
      description: listing.details, // Flaws, material, notes
      sellerEmail: seller?.email,
      sellerId: seller?.id
    });

    // Collect all unique photos (tag first, then item photos)
    const allPhotos = [];
    if (listing.photo_tag_url) {
      allPhotos.push(listing.photo_tag_url);
    }
    ...
    // Upload all unique photos to Shopify
