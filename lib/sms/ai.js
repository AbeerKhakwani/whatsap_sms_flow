...
export async function extractListingData(message, currentListing = {}) {
    ...
    try {
        const prompt = `Extract clothing listing data from this message. Be AGGRESSIVE at finding the designer brand.
...
{"designer": "Suffuse", "item_type": "3-piece", "size": "XL", "condition": "like new", "asking_price_usd": 90}
{"designer": "Ayesha's Boutique", "item_type": "kurta", "size": "M", "asking_price_usd": 75}
{"rejected_brand": "custom_made", "item_type": "lehnga", "size": "S"}
{"rejected_brand": "indian_brand", "item_type": "saree"}
 ...
 * Returns { isClothing: boolean, description?: string, hasBrandTag?: boolean }
 */
    ...
    try {
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You analyze photos for a clothing resale app. Be brief and friendly. Return JSON only.'
                },
                {
                    role: 'user',
                    content: [
                        {
                            type: 'text',
                            text: `Look at this photo. Is it clothing/fashion item?

Return JSON:
{
  "isClothing": true/false,
  "description": "brief friendly description if clothing, e.g. 'Beautiful purple kurta with gold embroidery'",
  ...
  "issue": "only if not clothing, e.g. 'That looks like food!'"
        ...
        return { isClothing: true, description: null, hasBrandTag: false };
    }
}
