...
async function getMetafieldDefinitions() {
  ...
  try {
    ...
    if (res.ok) {
      const { metafield_definitions } = await res.json();
      const map = {};
      for (const def of metafield_definitions) {
        map[`${def.namespace}.${def.key}`] = def.type?.name || def.type;
      }
      _metafieldDefsCache = map;
      console.log('Shopify metafield definitions:', map);
      return map;
    }
  } catch (e) {
...
export async function createDraft({ designer, itemType, size, condition, askingPrice, color, material, description, sellerEmail, sellerId, sellerPhone, commissionRate, chest, hip, notes, originalPrice }) {
  ...
  // Build metafields array using Shopify's defined types
  const metafields = [
  ...
  if (measurements) {
    metafields.push(buildMetafield(defs, 'custom', 'measurements', measurements, 'single_line_text_field'));
  }
  if (material) {
    metafields.push(buildMetafield(defs, 'custom', 'material', material, 'single_line_text_field'));
  }
  if (condition) {
    metafields.push(buildMetafield(defs, 'custom', 'condition', condition, 'single_line_text_field'));
  }
  if (notes || description) {
    metafields.push(buildMetafield(defs, 'custom', 'seller_description', notes || description || '', 'multi_line_text_field'));
  }
  if (retailPrice > 0) {
    metafields.push(buildMetafield(defs, 'custom', 'estimated_retail_price', JSON.stringify({ amount: retailPrice.toFixed(2), currency_code: 'USD' }), 'money'));
    const savings = retailPrice - listingPrice;
    if (savings > 0) {
      metafields.push(buildMetafield(defs, 'custom', 'your_savings', JSON.stringify({ amount: savings.toFixed(2), currency_code: 'USD' }), 'money'));
    }
  }
