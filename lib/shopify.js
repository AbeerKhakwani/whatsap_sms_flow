 ...
 */
export async function createDraft({ designer, itemType, size, condition, askingPrice, color, material, description, sellerEmail, sellerId, sellerPhone, commissionRate, chest, hip, notes, originalPrice, source }) {
  const { url, token } = getConfig();
  ...
  const product = {
    product: {
      ...
      tags: [designer, size, condition, color, material, 'preloved', 'pending-approval', source ? `source:${source}` : null].filter(Boolean).join(', '),
      options,
      variants: [variant],
      status: 'draft',
      metafields
