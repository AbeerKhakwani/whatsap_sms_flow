...
export async function createDraft({ designer, itemType, size, condition, askingPrice, color, material, description, sellerEmail, sellerId, sellerPhone, commissionRate, chest, hip, notes, originalPrice }) {
  ...
  // Build metafields — defaults match Shopify's actual definitions
  // (pricing fields are number_integer, text fields are multi_line_text_field)
  const metafields = [
    ...
    buildMetafield(defs, 'pricing', 'seller_asking_price', Math.round(sellerAsk).toString(), 'number_integer'),
    buildMetafield(defs, 'pricing', 'seller_payout', Math.round(sellerPayout).toString(), 'number_integer'),
  ];
