...
import { calculateSellerPayout, PLATFORM_FEE, DEFAULT_COMMISSION_RATE } from './payout-calculation.js';
...
// Pricing constants — the platform fee is the "listing fee" in the listing context.
const LISTING_FEE = PLATFORM_FEE; // $10 added to listing price
 ...
 */
/**
 * The seller's dry-cleaning answer decides whether the storefront may offer the
 * $20 dry-cleaning add-on (see dry-clean-addon.liquid, which hides itself on
 * 'no-dry-clean'). A seller who won't clean the item means we can't sell the
 * service on it, so that answer suppresses the checkbox.
 */
function dryCleanTag(dryCleaned) {
  switch (dryCleaned) {
    case 'Yes': return 'dry-cleaned';
    case 'Buyer pays': return 'dry-clean-on-request';
    case 'No': return 'no-dry-clean';
    default: return null;
  }
}

export async function createDraft({ designer, itemType, size, condition, askingPrice, color, material, description, sellerEmail, sellerId, sellerPhone, commissionRate, chest, hip, notes, originalPrice, source, concierge, dryCleaned, shipsFrom }) {
  const { url, token } = getConfig();
  ...
  // Payout via shared formula: (listingPrice − fee) × (1 − commission) = sellerAsk × (1 − commission)
  const { sellerPayout } = calculateSellerPayout({ grossPrice: listingPrice, commissionRate: commission });
  ...
  const metafields = [
    ...
    buildMetafield(defs, 'pricing', 'listing_fee', JSON.stringify({ amount: LISTING_FEE.toFixed(2), currency_code: 'USD' }), 'money'),
  ...
  if (dryCleaned) {
    metafields.push(buildMetafield(defs, 'custom', 'dry_cleaned', dryCleaned, 'single_line_text_field'));
  }
  // Every item must carry where it ships from — we only sell within North America,
  // and the buyer-facing delivery estimate depends on it.
  if (shipsFrom) {
    metafields.push(buildMetafield(defs, 'custom', 'ships_from', shipsFrom, 'single_line_text_field'));
  }
  if (notes || description) {
    metafields.push(buildMetafield(defs, 'custom', 'seller_description', notes || description || '', 'multi_line_text_field'));
  }
  // Two boolean mirrors of the `concierge` tag. `custom.concierge` is set only when true —
  // an explicit false would give the storefront filter a second value to render.
  // `custom.shipped_by_tps` is written either way as admin bookkeeping. See lib/listing-sync.js.
  if (concierge) {
    metafields.push(buildMetafield(defs, 'custom', 'concierge', 'true', 'boolean'));
  }
  metafields.push(buildMetafield(defs, 'custom', 'shipped_by_tps', concierge ? 'true' : 'false', 'boolean'));
  if (retailPrice > 0) {
  ...
  // The option value and the tag must both be canonical; the `custom.condition`
  // metafield above deliberately keeps the seller's raw wording (flaw disclosures).
  const canonCondition = canonicalCondition(condition) || 'Like New';
  const options = [
    { name: 'Size', values: [size || 'One Size'] },
    { name: 'Color', values: [color || 'Not specified'] },
    { name: 'Condition', values: [canonCondition] }
  ];
  const variant = {
    option1: size || 'One Size',
    option2: color || 'Not specified',
    option3: canonCondition,
  ...
  const product = {
    product: {
      ...
      tags: [designer, size, canonCondition, color, material, 'preloved', 'pending-approval', source ? `source:${source}` : null, concierge ? 'concierge' : null, dryCleanTag(dryCleaned)].filter(Boolean).join(', '),
      options,
      variants: [variant],
      status: 'draft',
      metafields
...
/**
 * Add a product image from a URL (Shopify downloads it directly)
 */
export async function addProductImageFromUrl(productId, imageUrl, filename) {
  const { url, token } = getConfig();

  console.log(`📸 Shopify URL upload: productId=${productId}, url=${imageUrl}`);
  ...
  if (!response.ok) {
    const error = await response.text();
    console.error(`❌ Shopify URL image upload failed: ${error}`);
    throw new Error(`Shopify image upload error: ${error}`);
  }

  const { image } = await response.json();
  console.log(`✅ Shopify image uploaded from URL: id=${image?.id}`);
  return image;
}
...
export async function getPendingDrafts() {
  ...
  // Fetch draft products (fresh submissions with pending-approval tag)
  // AND active products that have been revised by seller (seller-revised / needs-revision)
  // Revised products keep their active status — only their tag changes
  const [draftRes, revisedRes, needsRevisionRes] = await Promise.all([
    fetch(`https://${url}/admin/api/2024-10/products.json?status=draft&limit=50`, {
      headers: { 'X-Shopify-Access-Token': token }
    }),
    fetch(`https://${url}/admin/api/2024-10/products.json?status=active&tag=seller-revised&limit=50`, {
      headers: { 'X-Shopify-Access-Token': token }
    }),
    fetch(`https://${url}/admin/api/2024-10/products.json?status=active&tag=needs-revision&limit=50`, {
      headers: { 'X-Shopify-Access-Token': token }
    })
  ]);
  ...
  const { products: draftProducts } = await draftRes.json();
  const { products: revisedProducts } = revisedRes.ok ? await revisedRes.json() : { products: [] };
  const { products: needsRevisionProducts } = needsRevisionRes.ok ? await needsRevisionRes.json() : { products: [] };

  // Deduplicate by product id (in case a product somehow appears in multiple lists)
  const seen = new Set();
  const all = [];
  for (const p of [...draftProducts, ...revisedProducts, ...needsRevisionProducts]) {
    if (!seen.has(p.id)) {
      seen.add(p.id);
      all.push(p);
    }
  ...
  // Return products pending review: newly submitted OR revised by seller OR awaiting seller update
  return all.filter(p =>
    p.tags && (
      p.tags.toLowerCase().includes('pending-approval') ||
      p.tags.toLowerCase().includes('seller-revised') ||
      p.tags.toLowerCase().includes('needs-revision')
    )
  );
}
 ...
 */
/**
 * Create a Shopify draft order at a custom price (for accepted offers).
 * Returns { id, invoiceUrl } — invoiceUrl is the buyer's checkout link.
 */
export async function createDraftOrder({ variantId, price, buyerEmail, productTitle, offerId }) {
  const { url, token } = getConfig();
  ...
  const body = {
    draft_order: {
      ...
      email: buyerEmail,
      note: `Offer accepted — Offer ID: ${offerId}`,
      tags: 'offer-accepted',
      use_customer_default_address: false,
    }
  };

  const res = await fetch(`https://${url}/admin/api/2024-10/draft_orders.json`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`Draft order failed: ${JSON.stringify(err)}`);
  }
...
export async function approveDraft(productId) {
  ...
  // Use lowercase canonical gender tags — keeps consistency with cleanup tool
