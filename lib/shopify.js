...
export async function createDraft({ designer, itemType, size, condition, askingPrice, color, material, description, sellerEmail, sellerId, sellerPhone, commissionRate, chest, hip, notes, originalPrice }) {
  ...
  const metafields = [
    { namespace: 'seller', key: 'email', value: sellerEmail || '', type: 'single_line_text_field' },
    { namespace: 'seller', key: 'id', value: sellerId || '', type: 'single_line_text_field' },
    { namespace: 'seller', key: 'phone', value: sellerPhone || '', type: 'single_line_text_field' },
    { namespace: 'pricing', key: 'commission_rate', value: commission.toString(), type: 'number_integer' },
    { namespace: 'pricing', key: 'seller_asking_price', value: JSON.stringify({ amount: sellerAsk.toFixed(2), currency_code: 'USD' }), type: 'money' },
    { namespace: 'pricing', key: 'seller_payout', value: JSON.stringify({ amount: sellerPayout.toFixed(2), currency_code: 'USD' }), type: 'money' },
  ];

  // Map to existing custom.* Shopify metafields
  if (measurements) {
    metafields.push({ namespace: 'custom', key: 'measurements', value: measurements, type: 'single_line_text_field' });
  }
  if (material) {
    metafields.push({ namespace: 'custom', key: 'material', value: material, type: 'single_line_text_field' });
  }
  if (condition) {
    metafields.push({ namespace: 'custom', key: 'condition', value: condition, type: 'single_line_text_field' });
  }
  if (notes || description) {
    metafields.push({ namespace: 'custom', key: 'seller_description', value: notes || description || '', type: 'multi_line_text_field' });
  }
  if (retailPrice > 0) {
    metafields.push({ namespace: 'custom', key: 'estimated_retail_price', value: JSON.stringify({ amount: retailPrice.toFixed(2), currency_code: 'USD' }), type: 'money' });
    const savings = retailPrice - listingPrice;
    if (savings > 0) {
      metafields.push({ namespace: 'custom', key: 'your_savings', value: JSON.stringify({ amount: savings.toFixed(2), currency_code: 'USD' }), type: 'money' });
    }
  }

  // Build options and variant — include Color as option4 if provided
  const options = [
  ...
  if (color) {
    options.push({ name: 'Color', values: [color] });
    variant.option4 = color;
  }
