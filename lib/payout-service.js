 ...
 */
export async function payViaProvider(tx, { provider = 'zelle_manual', method, handle, reference, adminNote, payoutNotes, screenshotUrl, notify = true } = {}) {
  switch (provider) {
    // case 'stripe_express': return payViaStripe(tx, { ... });  // future — fills `reference` from the API
    case 'zelle_manual':
    default:
      return recordManualPaid(tx, { provider: 'zelle_manual', method, handle, reference, adminNote, payoutNotes, screenshotUrl, notify });
  }
}

async function recordManualPaid(tx, { provider, method, handle, reference, adminNote, payoutNotes, screenshotUrl, notify }) {
  const update = {
    status:         'paid',
    payout_status:  'paid',
  ...
  // Public URL of the payment screenshot (e.g. Zelle), shown to the seller on the receipt.
  if (screenshotUrl) update.payout_screenshot_url = screenshotUrl;
...
async function notifyPayoutSent(tx, { method, handle }) {
  ...
  const { subject, html } = payoutReceiptEmail(seller.name, tx.product_title, {
    ...
    screenshotUrl: tx.payout_screenshot_url,
  });
