...
const CANDIDATE_FIELDS = 'id, order_id, product_title, payout_status, payout_hold, shipping_status, review_responded_at, tracking_number, carrier, shipping_label_url, delivered_at, fulfilled_at, contest_status';
...
function fulfillmentUpdates(tx, order) {
  ...
  const delivered = fulfillments.some(f => f.shipment_status === 'delivered');
  ...
  if (delivered && !tx.delivered_at) updates.delivered_at = new Date().toISOString();
...
export async function runPayoutSync({ dryRun = false } = {}) {
  ...
  const results = { checked: 0, skipped: 0, updated: 0, released: 0, changes: [], errors: [] };
