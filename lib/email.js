...
// Minimal "sold" note. salePrice is already net of any discount, so the amount
// shown = salePrice − shipping fee. No commission/payout here — those are verified
// by an admin and sent on the paid receipt. Works even when commission is unknown.
export function itemSoldInlineEmail(sellerName, productTitle, { salePrice, platformFee = 10 }) {
  const sold = Math.max(0, (Number(salePrice) || 0) - (Number(platformFee) || 0));
  return {
    subject: `🎉 Your item sold! - ${productTitle}`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        <h1 style="color: #16a34a;">🎉 Congratulations${sellerName ? `, ${sellerName}` : ''}!</h1>
        <p>Your item <strong>${productTitle}</strong> sold for <strong>$${sold.toFixed(2)}</strong>!</p>
        ...
        <p>You'll be paid out by the end of the month, once delivery is confirmed.</p>
