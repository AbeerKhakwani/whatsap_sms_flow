...
import { calculateSellerPayout } from './payout-calculation.js';
 ...
 */
export function payoutNotificationEmail(sellerName, productTitle, amount, notes, screenshotUrl, reference) {
  return {
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f0fdf4; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
          ...
          ${reference ? `<p style="margin: 8px 0 0 0; color: #374151; font-size: 13px;">Confirmation #: <strong>${reference}</strong></p>` : ''}
        </div>
        ${screenshotUrl ? `<div style="margin: 20px 0; text-align: center;">
          <p style="margin: 0 0 8px 0; color: #6b7280; font-size: 13px;">Payment confirmation</p>
          <img src="${screenshotUrl}" alt="Payment screenshot" style="max-width: 100%; border-radius: 8px; border: 1px solid #e5e7eb;" />
        </div>` : ''}
  ...
  return {
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
        ...
        <p style="color: #374151;">
          You'll be paid out by the end of the month via your preferred payment method. If you haven't set up your payout method yet, please update it in your seller profile.
        </p>
        <div style="margin: 24px 0;">
...
// Minimal "sold" note — intentionally shows NO money. Sale price, commission and
// payout are all withheld here; the seller only ever sees figures on the verified
// paid receipt. Amount args are ignored (kept tolerant so existing callers that
// still pass pricing don't break). Single source for the sold email across every
// sale path (order-webhook, seller.js, shopify-webhook).
export function itemSoldInlineEmail(sellerName, productTitle) {
  return {
    subject: `🎉 Your item sold! - ${productTitle}`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        <h1 style="color: #16a34a;">🎉 Congratulations${sellerName ? `, ${sellerName}` : ''}!</h1>
        <p>Your item <strong>${productTitle}</strong> just sold!</p>
        ...
        <p>Once your sale is complete, we'll send your payout details and the full breakdown.</p>
...
export function salePriceConfirmedEmail(sellerName, productTitle, salePrice, platformFee, commissionRate, sellerPayout) {
  const { commissionAmount } = calculateSellerPayout({ grossPrice: salePrice, commissionRate, platformFee });
  return {
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        ...
        <table style="width:100%; border-collapse:collapse; margin: 16px 0;">
          ...
          <tr><td style="padding:6px 12px 6px 0; color:#6b7280;">Shipping fee</td><td style="text-align:right;">-$${platformFee.toFixed(2)}</td></tr>
...
// Part 3a — buyer delivery confirmation (one positive action + "email us for issues").
export function buyerConfirmEmail(productTitle, confirmUrl) {
  return {
    subject: `Did your order arrive okay? — The Phir Story`,
    html: `
      <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #16a34a;">Your order was delivered!</h2>
        <p style="font-size: 16px; color: #374151;">We hope you love <strong>${productTitle}</strong>. Tap below to confirm it arrived and you're happy — that's it!</p>
        <div style="margin: 24px 0;">
          <a href="${confirmUrl}" style="background: #16a34a; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; display: inline-block; font-size: 16px;">Yes, I got it 🎉</a>
        </div>
        <p style="font-size: 14px; color: #6b7280;">Any issues with your order? Just email <a href="mailto:admin@thephirstory.com">admin@thephirstory.com</a> and we'll sort it out.</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 20px 0;">
        <p style="color: #9ca3af; font-size: 12px;">The Phir Story — Pakistani Designer Resale</p>
      </div>
    `
...
// Part 3b — ask the buyer to rate the seller (~1 week after delivery).
export function sellerReviewEmail(productTitle, reviewUrl) {
  return {
    subject: `How was your experience? — The Phir Story`,
    html: `
      <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #111827;">Rate your seller</h2>
        <p style="font-size: 16px; color: #374151;">How was your experience buying <strong>${productTitle}</strong>? A quick rating helps other shoppers and your seller.</p>
        <div style="margin: 24px 0;">
          <a href="${reviewUrl}" style="background: #C91A2B; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; display: inline-block; font-size: 16px;">Leave a rating</a>
        </div>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 20px 0;">
        <p style="color: #9ca3af; font-size: 12px;">The Phir Story — Pakistani Designer Resale</p>
      </div>
...
// Paid receipt — full verified breakdown, sent when admin marks the payout paid.
export function payoutReceiptEmail(sellerName, productTitle, { salePrice, discount = 0, platformFee = 10, commissionRate, sellerPayout, method, handle, reference, screenshotUrl }) {
  const { commissionBase, commissionAmount } = calculateSellerPayout({ grossPrice: salePrice, commissionRate, platformFee });
  const listedPrice = (Number(salePrice) || 0) + (Number(discount) || 0);
  const row = (l, v, opts = {}) => `<tr${opts.border ? ' style="border-top:2px solid #e5e7eb;"' : ''}><td style="padding:6px 12px 6px 0;color:${opts.bold ? '#111827' : '#6b7280'};${opts.bold ? 'font-weight:bold;' : ''}">${l}</td><td style="text-align:right;${opts.bold ? 'font-weight:bold;font-size:20px;color:#16a34a;' : ''}">${v}</td></tr>`;
  const paidVia = method ? ` via ${method}${handle ? ` (${handle})` : ''}` : '';
  return {
    subject: `💸 Payment sent — ${productTitle} ($${(Number(sellerPayout) || 0).toFixed(2)})`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        <h1 style="color:#16a34a;">💸 Your payment is on its way${sellerName ? `, ${sellerName}` : ''}!</h1>
        <p>We've sent your payout for <strong>${productTitle}</strong>${paidVia}. Here's the full breakdown:</p>
        <table style="width:100%;border-collapse:collapse;margin:16px 0;">
          ${row('Listed price', `$${listedPrice.toFixed(2)}`)}
          ${(Number(discount) || 0) > 0 ? row('Discount', `-$${Number(discount).toFixed(2)}`) : ''}
          ${row('Shipping fee', `-$${Number(platformFee).toFixed(2)}`)}
          ${row('Net (commission base)', `$${commissionBase.toFixed(2)}`)}
          ${row(`Commission (${commissionRate ?? '—'}%)`, commissionAmount != null ? `-$${commissionAmount.toFixed(2)}` : '—')}
          ${row('Your payout', `$${(Number(sellerPayout) || 0).toFixed(2)}`, { bold: true, border: true })}
        </table>
        ${reference ? `<p style="color:#374151;font-size:14px;">Confirmation #: <strong>${reference}</strong></p>` : ''}
        ${screenshotUrl ? `
        <p style="color:#6b7280;font-size:14px;margin-bottom:4px;">Payment confirmation:</p>
        <a href="${screenshotUrl}"><img src="${screenshotUrl}" alt="Payment confirmation screenshot" style="max-width:100%;border:1px solid #e5e7eb;border-radius:8px;" /></a>` : ''}
        <p style="color:#6b7280;font-size:14px;">Thank you for selling with The Phir Story!</p>
      </div>
    `
  };
}
