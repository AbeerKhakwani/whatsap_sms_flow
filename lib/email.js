...
// salePrice is the amount the buyer actually paid (already net of any discount).
// discount is shown for transparency; the listed price = salePrice + discount.
export function itemSoldInlineEmail(sellerName, productTitle, { salePrice, discount = 0, commissionRate, platformFee = 10, sellerPayout }) {
  const { commissionBase, commissionAmount } = calculateSellerPayout({ grossPrice: salePrice, commissionRate, platformFee });
  const listedPrice = salePrice + (discount || 0);
  const row = (label, value, opts = {}) => `
    <tr${opts.border ? ' style="border-top:2px solid #e5e7eb;"' : ''}>
      <td style="padding:6px 12px 6px 0; color:${opts.bold ? '#111827' : '#6b7280'}; ${opts.bold ? 'font-weight:bold;' : ''}">${label}</td>
      <td style="text-align:right; ${opts.bold ? 'font-weight:bold; font-size:20px; color:#16a34a;' : ''}">${value}</td>
    </tr>`;
  ...
  return {
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        ...
        <p>Your item <strong>${productTitle}</strong> just sold! Here's the breakdown:</p>
        <table style="width:100%; border-collapse:collapse; margin: 16px 0;">
          ${row('Listed price', `$${listedPrice.toFixed(2)}`)}
          ${(discount || 0) > 0 ? row('Discount', `-$${discount.toFixed(2)}`) : ''}
          ${row('Shipping fee', `-$${platformFee.toFixed(2)}`)}
          ${row('Net (commission is calculated on this)', `$${commissionBase.toFixed(2)}`)}
          ${row(`Commission (${commissionRate}%)`, `-$${commissionAmount.toFixed(2)}`)}
          ${row('Your payout', `$${sellerPayout.toFixed(2)}`, { bold: true, border: true })}
        </table>
        ${shippingSection}
        <p>You'll be paid out by the end of the month, once we've confirmed delivery.</p>
