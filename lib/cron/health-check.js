...
async function checkWhatsAppFlow() {
  // Vercel functions can't reliably self-fetch their own domain, so we
  // verify the private key can decrypt instead of hitting the HTTP endpoint.
  ...
  if (pubKey.type !== 'public') throw new Error('Key validation failed');
  return 'private key valid + crypto operational';
}
...
async function checkShopify() {
  const SHOP = process.env.SHOPIFY_SHOP || process.env.SHOPIFY_STORE || process.env.VITE_SHOPIFY_STORE_URL || process.env.VITE_SHOPIFY_STORE;
  const TOKEN = process.env.SHOPIFY_ACCESS_TOKEN || process.env.VITE_SHOPIFY_ACCESS_TOKEN;
  if (!SHOP || !TOKEN) throw new Error('Missing SHOPIFY_SHOP or SHOPIFY_ACCESS_TOKEN');
...
// Private key check is now part of checkWhatsAppFlow (validates crypto too)
...
export default async function handler(req, res) {
  ...
  const checks = await Promise.all([
    checkWithTimeout('WhatsApp Flow + Key', checkWhatsAppFlow),
