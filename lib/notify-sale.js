...
export async function notifySellerItemSold({ seller, productTitle, salePrice }) {
  ...
  // Slack heads-up (#dashboard) — automatic, no actor. No-op until SLACK_WEBHOOK_URL is set.
  await notifySlack(`🛒 Sold: "${productTitle}"${price > 0 ? ` — $${price.toFixed(2)}` : ''}${seller.name ? ` · ${seller.name}` : ''}`);
}
