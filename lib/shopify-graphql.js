...
export async function uploadToStagedUrl(uploadUrl, uploadParameters, fileBuffer) {
  // Use axios with form-data for proper multipart handling
  ...
  // Use Buffer directly with proper content type and filename
  ...
  try {
    // For pre-signed URLs, let axios/form-data handle headers automatically
    // DO NOT manually set headers - it breaks the signature
    ...
    console.error('❌ Upload error details:', error.response?.status, error.code);
    const errorMsg = error.response?.data || error.message;
    const status = error.response?.status || 'unknown';
    throw new Error(`Staged upload failed: ${status} - ${errorMsg}`);
  }
}
