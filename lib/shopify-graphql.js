...
/**
 * Bulk-fetch every product with its authoritative seller metafields (seller.id / seller.email),
 * paginating 250 at a time. Used by the ownership reconcile — one query per ~250 products, so it
 * never hits REST per-product rate limits or function timeouts.
 *
 * @returns {Promise<Array<{ id: string, title: string, status: string, sellerEmail: string|null, sellerId: string|null }>>}
 *          `id` is the bare numeric product ID (GID stripped), matching how shopify_product_ids stores them.
 */
export async function getAllSellerMetafields() {
  const out = [];
  let cursor = null;
  let hasNext = true;
  let pages = 0;
  ...
  while (hasNext && pages < 100) {
        ...
        products(first: 250, after: $cursor) {
          ...
          edges {
            node {
              ...
              sellerId: metafield(namespace: "seller", key: "id") { value }
            }
          }
        }
      }`,
    ...
    for (const { node } of conn.edges) {
      out.push({
        id: String(node.id).split('/').pop(), // gid://shopify/Product/123 -> "123"
        title: node.title,
        status: node.status,                   // ACTIVE | ARCHIVED | DRAFT
        sellerEmail: node.sellerEmail?.value || null,
        sellerId: node.sellerId?.value || null,
      });
    }
...
export async function createStagedUpload(filename, mimeType, fileSize) {
  ...
  console.log(`📋 Staged upload parameters:`, JSON.stringify(target.parameters));
...
export async function uploadToStagedUrl(uploadUrl, uploadParameters, fileBuffer) {
  // Google Cloud Storage pre-signed URLs expect a simple PUT request
  // NOT multipart form-data - just the raw file buffer
  // All parameters are already in the URL query string

  console.log(`📤 Uploading file to staged URL (${fileBuffer.length} bytes)...`);
  console.log(`📋 Upload parameters: ${uploadParameters.length} params`);

  try {
    // Simple PUT request with raw file buffer
    ...
    console.log(`✅ File uploaded to staged URL (status: ${response.status})`);
  } catch (error) {
...
export async function createProductWithMedia(productData, fileIds) {
  // Step 1: Create product (Shopify auto-creates default variant)
  const mutation = `
  ...
  // Step 2: Update default variant with price
  const variantId = product.variants.edges[0]?.node.id;
  if (variantId && productData.asking_price_usd) {
    await updateVariantPrice(variantId, productData.asking_price_usd);
  }
...
async function updateVariantPrice(variantId, price) {
  ...
  if (data.productVariantUpdate.userErrors.length > 0) {
    const error = data.productVariantUpdate.userErrors[0];
    console.error(`⚠️ Variant update error: ${error.message}`);
    // Don't throw - product is created, just log the warning
  } else {
    console.log(`✅ Variant price updated: $${price}`);
  }
}
