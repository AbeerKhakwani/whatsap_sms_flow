...
import { canonicalSize, isSizeTag } from './sizes.js';
...
export async function scanCompleteness({ status = 'active' } = {}) {
  ...
  for (const p of garments) {
    ...
    items.push({
      ...
      // Flag only — no size migration is applied. Surfaced on the once-over panel so a
      // reviewer can see when the variant says "M" and the tag says "Medium".
      sizeTagsDisagree: (() => {
        const ov = sizeOpt?.optionValues?.[0]?.name;
        if (!ov) return false;
        const st = p.tags.filter(isSizeTag);
        if (!st.length) return false;
        const want = canonicalSize(ov);
        return !st.some(t => canonicalSize(t) === want);
      })(),
...
// Namespaces owned by installed apps, not by us.
const APP_NAMESPACE = /^(judgeme|loox|reviews|mm-google-shopping|mc-facebook|ecomposer|auctionplus|shopify--discovery|circle-hand)\./;

const DETAIL = `query($id:ID!){ product(id:$id){
  id title handle status vendor productType tags
  category{ name }
  options{ id name position optionValues{ name linkedMetafieldValue } }
  variants(first:20){ edges{ node{ id title sku price compareAtPrice inventoryQuantity
    selectedOptions{ name value } } } }
  metafields(first:100){ edges{ node{ namespace key value type } } } } }`;

/**
 * Full current state of one listing — variants, every tag, every metafield — for the
 * once-over panel on a cleanup card. Fetched per card rather than in the bulk scan,
 * which would otherwise carry ~250 products' worth of metafields in one response.
 */
export async function getListingDetail(id) {
  const gid = String(id).startsWith('gid://') ? String(id) : `gid://shopify/Product/${id}`;
  const { product: p } = await gql(DETAIL, { id: gid });
  if (!p) return null;
  return {
    id: p.id.split('/').pop(), title: p.title, handle: p.handle, status: p.status,
    vendor: p.vendor, productType: p.productType, category: p.category?.name || null,
    tags: p.tags,
    options: p.options.map(o => ({ name: o.name, position: o.position,
      values: o.optionValues.map(v => ({ name: v.name, linked: Boolean(v.linkedMetafieldValue) })) })),
    variants: p.variants.edges.map(e => ({ id: e.node.id.split('/').pop(), title: e.node.title,
      sku: e.node.sku, price: e.node.price, compareAtPrice: e.node.compareAtPrice,
      inventory: e.node.inventoryQuantity,
      options: e.node.selectedOptions.map(o => `${o.name}: ${o.value}`) })),
    // Split ours from the installed apps'. Review widgets and feed apps store HTML blobs
    // that are noise on a review panel, so they are counted, not listed.
    metafields: p.metafields.edges
      .map(e => ({ key: `${e.node.namespace}.${e.node.key}`, value: e.node.value, type: e.node.type }))
      .filter(m => String(m.value ?? '').trim() !== '' && !APP_NAMESPACE.test(m.key))
      .sort((a, b) => a.key.localeCompare(b.key)),
    appMetafieldCount: p.metafields.edges
      .filter(e => APP_NAMESPACE.test(`${e.node.namespace}.${e.node.key}`)
        && String(e.node.value ?? '').trim() !== '').length,
  };
}
