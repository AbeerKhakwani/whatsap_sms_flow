...
export async function createShippoLabel(seller, productTitle, buyerAddress) {
  ...
  if (!shipmentRes.ok || shipment.messages?.length) {
    const errorMsg = shipment.messages?.[0]?.text || shipment.detail || 'Failed to create Shippo shipment';
    console.error('📦 Shippo shipment error:', JSON.stringify(shipment));
    throw new Error(errorMsg);
  }

  // Step 2: Pick best rate — prefer USPS Priority
  const rates = shipment.rates || [];
  if (rates.length === 0) {
    throw new Error('No shipping rates available');
  }

  const priorityRate = rates.find(r => r.servicelevel?.token === 'usps_priority')
    || rates.find(r => r.provider === 'USPS')
...
export async function getShippingLabel(seller, productTitle, buyerAddress = null) {
  ...
  // LAST RESORT: manual shipping instructions
  return {
    type: 'instructions',
