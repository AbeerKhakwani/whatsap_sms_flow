...
export async function createEasyshipLabel(seller, productTitle, buyerAddress) {
  ...
  const response = await fetch(`${baseUrl}/2023-01/shipments`, {
    method: 'POST',
    headers: {
  ...
  // Get rates
  const ratesRes = await fetch(`${baseUrl}/2023-01/shipments/${shipmentId}/rates`, {
    headers: {
      'Authorization': `Bearer ${EASYSHIP_API_KEY}`
    }
  });
  ...
  // Buy the label
  const buyRes = await fetch(`${baseUrl}/2023-01/shipments/${shipmentId}/buy_label`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${EASYSHIP_API_KEY}`,
      'Content-Type': 'application/json'
