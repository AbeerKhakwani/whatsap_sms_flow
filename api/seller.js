  ...
  try {
    ...
    if (action === 'shipping-label' && req.method === 'POST') {
      ...
      try {
        ...
        // Notify admin that label was generated
        await sendEmail({
          to: process.env.ADMIN_EMAIL || 'thephirstory@gmail.com',
          subject: `📦 Label generated: ${productTitle}`,
          html: `<p>A shipping label was generated by seller <strong>${seller.name}</strong> (${seller.email}).</p>
                 <p><strong>Item:</strong> ${productTitle}<br>
                 <strong>Tracking:</strong> ${labelResult.trackingNumber}<br>
                 <strong>Carrier:</strong> ${labelResult.carrier} ${labelResult.service}<br>
                 <strong>Rate:</strong> $${labelResult.rate}<br>
                 <strong>Est. Delivery:</strong> ${labelResult.estimatedDelivery}</p>
                 <p><a href="${labelResult.labelUrl}">View Label</a></p>`,
          context: 'label_generated'
        }).catch(e => console.error('Admin label notification failed:', e.message));
        ...
        // Alert admin — seller is stuck and can't ship
        await sendEmail({
          to: process.env.ADMIN_EMAIL || 'thephirstory@gmail.com',
          subject: `🚨 Label generation failed: ${productTitle || 'Unknown item'}`,
          html: `<p>Shipping label generation failed for seller <strong>${seller?.name || email}</strong> (${email}).</p>
                 <p><strong>Item:</strong> ${productTitle || 'Unknown'}<br>
                 <strong>Transaction ID:</strong> ${transactionId || 'N/A'}<br>
                 <strong>Error:</strong> ${err.message}</p>
                 <p>The seller cannot ship until this is resolved. Please generate the label manually or contact the seller.</p>`,
          context: 'label_failed'
        }).catch(() => {});
        return res.status(200).json({
          success: false,
          labelFailed: true,
    ...
    if (action === 'shippo-webhook' && req.method === 'POST') {
      ...
      const { data: tx } = await supabase
        .from('transactions')
        .select('id, payout_status, product_title, seller_id, sellers(name, email)')
      ...
      switch (status) {
        ...
        case 'FAILURE':
          ...
          updates.admin_note = `⚠️ Package delivery failed — tracking ${trackingNumber}. Manual action needed.`;
          break;
        case 'RETURNED':
          updates.shipping_status = 'returned';
          updates.admin_note = `⚠️ Package returned to sender — tracking ${trackingNumber}. Manual action needed.`;
          break;
        default:
          return res.status(200).json({ ok: true });
      }
      ...
      if (status === 'FAILURE' || status === 'RETURNED') {
        const sellerName = tx.sellers?.name || 'Unknown seller';
        const sellerEmail = tx.sellers?.email || 'Unknown';
        const statusLabel = status === 'FAILURE' ? 'delivery failed' : 'returned to sender';
        await sendEmail({
          to: process.env.ADMIN_EMAIL || 'thephirstory@gmail.com',
          subject: `🚨 Package ${statusLabel}: ${tx.product_title || trackingNumber}`,
          html: `<p>A package has <strong>${statusLabel}</strong> via USPS.</p>
                 <p><strong>Item:</strong> ${tx.product_title || 'Unknown'}<br>
                 <strong>Tracking:</strong> ${trackingNumber}<br>
                 <strong>Seller:</strong> ${sellerName} (${sellerEmail})</p>
                 <p>Please investigate and contact the seller. The payout pipeline is paused until resolved.</p>`,
          context: 'shipping_problem'
        }).catch(e => console.error('Admin shipping alert failed:', e.message));
      }
